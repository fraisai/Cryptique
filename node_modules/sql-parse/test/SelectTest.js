...
describe('parse select sql',function(){
  ...
  /*{{{ test parse entire sql with hint works fine*/
  it('test parse entire sql with hint works fine',function(done){
    var sql = "sElect /*i am hint*/distInct a.m as Afrom from sql.(select * from M) as bjoin rIght JoIn action.joinTab as tab On tab.col>b.col where b.id in (1,4,6) grouP by b.id Order bY b.order LimIt 2,17";
    var expect = {
      hint : {text:"i am hint",type:99},
      column:{
        Afrom : {
          dist : {text:"DISTINCT",type:1},
          expr : [{text:"a.m",type:1}]
